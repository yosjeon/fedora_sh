...
cat <<__EOF__
#--
#-- 표시할 주의 갯수를 입력하세요. Enter 만 입력하면 [ ${ju_cnt} ] 주간 을 표시합니다.
__EOF__
read total_ju
...
if [[ todays_week -ne 0 ]]; then #-- %w .... day of week (0..6); 0 is Sunday
	yy=$(date +%y --date='TZ="Asia/Seoul" 00:01 last Sun')
	mm=$(date +%m --date='TZ="Asia/Seoul" 00:01 last Sun')
	dd=$(date +%d --date='TZ="Asia/Seoul" 00:01 last Sun')
else
	yy=$(date +%y --date='TZ="Asia/Seoul" 00:01  Sun')
	mm=$(date +%m --date='TZ="Asia/Seoul" 00:01  Sun')
	dd=$(date +%d --date='TZ="Asia/Seoul" 00:01  Sun')
fi
...
cat <<__EOF__
# $(LC_TIME=C date +%y%m%d_%a).${yoil:${todays_week}:1}
$(date +%H%M) / ${work_tab:${today_work}:2} / 1639.2653 / 100.158 / 
...
last_dd=$(date -d "$(date +%Y-%m-01) - 1 day" +%d)
