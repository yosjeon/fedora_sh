...
1 ${show_1}
2 ${show_2}
#---> Enter: [ ${enter_zero_or_one_two} ] #-- 0, 1 또는 2
__EOF__
read a
...
if [[ "x${enter_zero_or_one_two}" == "x0" ]]; then
	...
	if [ "x${enter_zero_or_one_two}" == "x1" ]; then
		show_0_1_2="${show_1}"
		dir_000102="01"
		dirName="${dir_000102}-last_big_files"
		abc_is="0b"
		...
		if [ "x${enter_zero_or_one_two}" == "x2" ]; then
			show_0_1_2="${show_2}"
			dir_000102="02"
			dirName="${dir_000102}-more_300M_files"
			abc_is="0c"
...
echo "${uuu}"
cat <<__EOF__
${rrr}[ ${yyy}${enter_zero_or_one_two} ${rrr}]${bbb}${show_0_1_2}${xxx}
...
#-- 이름순
cmdrun "ls -l ${dirName}/ | awk -v dors="${enter_d_or_s}" 'NR > 1 { file_type = substr(\$1, 1, 1); printf \"%s %s %12s %5s %2s %5s %s \", file_type, dors, \$5, \$6, \$7, \$8, \$9; printf \"\\n\" }' > ${seq_dir_ymdHM_7z}.ls-l" "(2) 이름순 ${seq_dir_ymdHM_7z}.ls-l 목록을 만듭니다."

#-- 기타 = 시간순
cmdrun "ls -lrt ${dirName}/ | awk -v dors="${enter_d_or_s}" 'NR > 1 { file_type = substr(\$1, 1, 1); printf \"%s %s %12s %5s %2s %5s %s \", file_type, dors, \$5, \$6, \$7, \$8, \$9; printf \"\\n\" }' > ${seq_dir_ymdHM_7z}.ls-lrt" "(3) ${seq_dir_ymdHM_7z}.ls-lrt 시간순 목록을 만듭니다." #-- 0[abc][DS]-0[012]-
...
cmdrun "ls -lrt > ${local_lrt}; tail -9 ${seq_dir_ymdHM_7z}*t; echo \"---\"; tail -9 ${local_lrt}" "(6) 다운로드 완료후 파일을 비교합니다."
