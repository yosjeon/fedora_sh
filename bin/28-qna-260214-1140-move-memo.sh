...
echo ""
a="gemini"
echo "#-- (1) ai 이름 [ ${a} ] ----> bash 로 실행해야 합니다."
read b
if [ "x$b" = "x" ]; then
    b="$a"
fi
...
echo "#-- (2) 시작 일련번호 [ ${a} ] ----> bash 로 실행해야 합니다."
read b
if [ "x$b" = "x" ]; then
    b="$a"
fi
...
#--
a=$(( $bg_no + 10 ))
echo "#-- (3) 끝 일련번호 [ ${a} ] ----> bash 로 실행해야 합니다."
read b
if [ "x$b" = "x" ]; then
    b="$a"
fi
...
echo "#-- (4) 토론의 주제 입력 또는, press Enter: [ ${a} ] ----> bash 로 실행해야 합니다."
read b
if [ "x$b" = "x" ]; then
    b="$a"
fi
...
#-- echo "#-- h2m2 HHMM;"
a=${y4md}
echo "#-- (5) 등록하는 년도4자리+월일 [ ${a} ] 8 자리 ----> bash 로 실행해야 합니다."
read b
if [ "x$b" = "x" ]; then
    b="$a"
fi
...
echo "#-- (6) 등록하는 시분 [ ${a} ] 4 자리 ----> bash 로 실행해야 합니다."
read b
if [ "x$b" = "x" ]; then
    b="$a"
fi
a="n"
echo "#-- (7) mkdir + rsync 명령 추가면 'y', 아니면 [ $a ] ----> bash 로 실행해야 합니다."
read mr_y
if [ "x$mr_y" = "x" ]; then
    mr_y="$a" #-- Enter = n
fi
