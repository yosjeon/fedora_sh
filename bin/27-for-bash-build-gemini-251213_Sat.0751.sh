...
cat <<__EOF__
# sh $0 gemcli "qna 를 기록하는 메모장 작성" 00
#----------------------------------------------- $1 ---- $2 ------------------------- $3
...
do
    ...
    JOB_DATE_SEQ="${JOB_CODE}-${SEQ_NUM}" # 최종 문자열 포맷
...
3,.  w  ${SEQ_NUM}a-${JOB_CODE}.txt  파일읽고 진행한 결과를 ${SEQ_NUM}b-${JOB_CODE}.txt  파일에 써줘.   #---
   ----------
mkdir -p ~/Downloads/${WINDOWS_DIR}; cd ~/Downloads/${WINDOWS_JOBCODE}*
rsync -avzr -e 'ssh -p 5822' proenpi@pi:a*/m*/g*/${SEQ_NUM}q-*.txt ./; ll #-- 서버에서 만든 txt 를 geminicli 를 설치한 로컬로 복사하는 스크립트.
__EOF__
    else
        cat >> ../${LAST_99_MD_NAME} <<__EOF__
3,.  w  ${SEQ_NUM}a-${JOB_CODE}.txt  파일읽고 진행한 결과를 ${SEQ_NUM}b-${JOB_CODE}.txt  파일에 써줘.   #---
   ----------
rsync -avzr -e 'ssh -p 5822' proenpi@pi:a*/m*/g*/${SEQ_NUM}a-*.txt ./; ll #-- 서버에서 만든 txt 를 geminicli 를 설치한 로컬로 복사하는 스크립트.
__EOF__
    fi
    cat >> ../${LAST_99_MD_NAME} <<__EOF__
...
echo "#// 작성한 ../${LAST_99_MD_NAME} 파일 확인."
echo "#--"
echo "#-- 작업할 곳:  cd ${WINDOWS_JOBCODE}* ; pwd ; ls -l"
cd ${WINDOWS_JOBCODE}* ; pwd ; ls -l
echo "#// 작업할 곳:  cd ${WINDOWS_JOBCODE}* ; pwd ; ls -l"
echo "#--"
echo "#-- vi ../last*99*md"
vi ../last*99*md
